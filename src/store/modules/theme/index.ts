...
export const useThemeStore = defineStore(SetupStoreId.Theme, () => {
  ...
  const themeColors = computed(() => {
    ...
    // In dark mode, if the theme color is black, it will be invisible.
    // We force it to white to ensure visibility and valid contrast.
    const isBlack = themeColor.toLowerCase() === '#000000'
    const effectiveThemeColor = darkMode.value && isBlack ? '#ffffff' : themeColor

    const colors: App.Theme.ThemeColor = {
      primary: effectiveThemeColor,
      ...otherColor,
      info: isInfoFollowPrimary ? effectiveThemeColor : otherColor.info
    }
    return colors
  })
